...
public:
    ...
    inline void reset(int n, int m)
    {
        ...
        m_theta = Scalar(1);
        m_s.resize(n, m);
        m_s.setZero(); // <- added this, otherwise random numbers, sometimes, crazy big
        m_y.resize(n, m);
        m_ys.resize(m);
        m_alpha.resize(m);
        m_ncorr = 0;
    ...
    // reset hessian if we don't trust it anymore
    inline void resetHess()
    {
        m_s.setZero();
        m_ptr = m_m;
    }
    ...
    inline void add_correction(const RefConstVec& s, const RefConstVec& y, int mpi_rank)
    {
